...
class fpt_apb_slave_monitor extends uvm_monitor;
    ...
    extern function new(
        string        name = "fpt_apb_slave_monitor",
...
function fpt_apb_slave_monitor::new(
    string        name = "fpt_apb_slave_monitor",
    uvm_component parent = null
);
...
function void fpt_apb_slave_monitor::build_phase(uvm_phase phase);
    ...
    if (!uvm_config_db#(fpt_apb_vif_t)::get(
        ...
        `uvm_fatal(
            get_full_name(),
            "No virtual interface specified for fpt_apb_slave_monitor"
        )
    end
    ...
    forever begin
        ...
        do begin
            @(vif.slave_mon_cb);
                   ...
                   vif.slave_mon_cb.PSEL !== 1'b1 ||
                   vif.slave_mon_cb.PENABLE !== 1'b0);
        ...
        item.PADDR   = vif.slave_mon_cb.PADDR;
        item.PWRITE  = tx_type_e'(vif.slave_mon_cb.PWRITE);
        item.PWDATA  = vif.slave_mon_cb.PWDATA;
        item.PSTRB   = vif.slave_mon_cb.PSTRB;
        ...
        forever begin
            ...
            if (vif.slave_mon_cb.PSEL !== 1'b1 ||
                vif.slave_mon_cb.PENABLE !== 1'b1) begin
                `uvm_error(
                    "APB_MONITOR",
            ...
            if (vif.slave_mon_cb.PREADY === 1'b1) begin
                item.PRDATA  = vif.slave_mon_cb.PRDATA;
                item.PSLVERR = slave_error_e'(
                    vif.slave_mon_cb.PSLVERR
                );
