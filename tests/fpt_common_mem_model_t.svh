`ifndef FPT_COMMON_MEM_MODEL_T_SVH
`define FPT_COMMON_MEM_MODEL_T_SVH
...
class fpt_common_mem_model_t extends uvm_object;
    `uvm_object_utils(fpt_common_mem_model_t)
    ...
    extern function new(string name = "fpt_common_mem_model_t", 
...
endclass: fpt_common_mem_model_t
...
function fpt_common_mem_model_t::new(string name = "fpt_common_mem_model_t", int unsigned fpt_base_addr = 32'h0, int unsigned fpt_addr_range = 32'hFFFF);
    super.new(name);
    this.fpt_base_addr  = fpt_base_addr;
    this.fpt_addr_range = fpt_addr_range;
endfunction: new

function void fpt_common_mem_model_t::do_print(uvm_printer printer);
...
function void fpt_common_mem_model_t::do_copy(uvm_object rhs);
    fpt_common_mem_model_t rhs_;
...
function bit fpt_common_mem_model_t::do_compare(uvm_object rhs, uvm_comparer comparer);
    fpt_common_mem_model_t rhs_;
    bit status = 1;
...
function void fpt_common_mem_model_t::fpt_init_mem_model(FPT_MEMORY_INIT_PATTERN_E fpt_pattern = ALL0);
...
function void fpt_common_mem_model_t::fpt_write(int unsigned fpt_addr, byte data [3:0], bit [3:0] fpt_mem_pstrobe);
...
function void fpt_common_mem_model_t::fpt_write_32(int unsigned fpt_addr, byte data [3:0]);
...
function void fpt_common_mem_model_t::fpt_write_8(int unsigned fpt_addr, byte data);
    fpt_mem_array[fpt_addr] = data;
endfunction: fpt_write_8

function void fpt_common_mem_model_t::fpt_read_32(int unsigned fpt_addr, output byte data [3:0]);
...
function void fpt_common_mem_model_t::fpt_read_8(int unsigned fpt_addr, output byte data);
    data = fpt_mem_array.exists(fpt_addr) ? fpt_mem_array[fpt_addr] : 8'h00;
endfunction: fpt_read_8

`endif // fpt_common_mem_model_t_SVH
